...
echo -e "\n==== Copying files to Pi ====\n"
rsync -av -e "ssh -o StrictHostKeyChecking=no" --delete --exclude={'.git','.gitignore','commands.txt','README.md','node_modules'} $(pwd) $USER@$PI_HOST:/home/$USER

# Use SSH to execute commands on the Pi server
echo -e "\n==== Executing install script ====\n"
ssh -o StrictHostKeyChecking=no  $USER@$PI_HOST 'cd relativepath && bash nginx_install.sh && bash api_deploy.sh && bash service.sh' # && bash api_deploy.sh && bash service.sh
